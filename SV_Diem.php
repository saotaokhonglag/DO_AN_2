...
$SV = $conn -> query("SELECT * FROM sinhvien WHERE MSSV = '$username'");
    if(mysqli_num_rows($SV)>0)
    {
    ...
    $sql = $conn -> query("SELECT lh.TenLop, lh.MaLopHP, ct.DiemGK, ct.DiemCK, ct.DIemTB, pc.HocKi, pc.NamHoc
                            FROM chitietmh ct, sinhvien sv, phancong pc, lophp lh
                            WHERE lh.MaLopHP = pc.MaLopHP AND
                                ct.MaLopHP = lh.MaLopHP AND
                                ct.MSSV = sv.MSSV AND
                                sv.MSSV = '$username'
                                ORDER BY pc.HocKi, pc.NamHoc DESC "); 
?>
     ...
     <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3" style = "text-align: center;">
                <h5 class="m-0 font-weight-bold text-primary">KẾT QUẢ HỌC TẬP</h5>
            </div>
            <div class="card-body">
                ...
                <form method="POST" action ="PageSV.php">
                    <table class="table table-bordered" style = "text-align: center;" id="dataTable" width="100%" cellspacing="0">
                        <tr>
                                ...
                                <th>Học kì</th>
                                <th>Năm học</th>
                                <th colspan="2">Giữa kì</th>
                                <th colspan="2">Cuối kì</th> 
                                <th>Trung bình</th>
                                <th>Xếp loại</th>  
                                <th>Ghi chú</th> 
                        </tr>
                        ...
                        ?>   
                            <tr> 
                                <th><?php echo $STT++;?></th>
                                <th><?php echo $rows['TenLop'] ;?></th>
                                <th><?php echo $rows['MaLopHP'] ;?></th>
                                <th><?php echo $rows['HocKi'] ;?></th>
                                <th><?php echo $rows['NamHoc'] ;?></th>
                                <th><?php echo $rows['DiemGK'];?> </th>
                                <th>
                                    <?php 
                                ...
                                <th><?php echo $rows['DIemTB'];?> </th>
                                <th> 
                                    <?php
                                       if($rows['DIemTB'] >= '8.5' && $rows['DIemTB'] <= '10.0')
                                       {
                                           echo 'Giỏi';
                                       }
                                       if($rows['DIemTB'] >= '6.5' && $rows['DIemTB'] <= '8.4')
                                       {
                                           echo 'Khá';
                                       }
                                       if($rows['DIemTB'] >= '5.0' && $rows['DIemTB'] <= '6.4')
                                       {
                                           echo 'Trung bình';
                                       }
                                       if($rows['DIemTB'] >= '4.0' && $rows['DIemTB'] <= '4.9')
                                       {
                                           echo 'Yếu';
                                       }
                                       if($rows['DIemTB'] <= '3.9')
